...
const fetchBtnCategory = async () => {
  ...
  allData.forEach((item) => {
    ...
    const btn = `<button onclick="handleClick(${item.category_id})" class="btn lg:w-[100px] md:w-[100px] w-full capitalize m-3">${item.category}</button>`
    div.innerHTML = btn;

    categoryesBtnContainer.appendChild(div);
  })
}
...
const showCards = (data, isSortByView) => {
  ...
  if(data.length === 0){
    ...
    const errorPageContent = `
      <div class=" flex items-center justify-center flex-col mt-3 w-[95vw]">
        <img src="../Icon.png"/>
        <h3 class="text-2xl font-bold">Opps!There no data in this category!</h3>
      </div>
    `;
    errorPage.innerHTML = errorPageContent;
    return cardsContainer.appendChild(errorPage)
  }
      ...
      <div class="card-body p-3 items-start text-start">
        <div class="flex">
          ...
          <div class="mx-1">
            ...
            <div>
              <p class="p-0 m-0 flex items-center">${item.authors[0].profile_name}
                <span class="pl-[4px]">${item.authors[0].verified === false ? '' : `
